...
- How far to trust waiver buckets (2025 backtest, docs/BACKTEST.md): start_now pickups beat the starter they replace (+10 to +15 pts over 3 weeks, depending on whether other managers claim players), and roughly tie the top-projected free agent when other managers don't claim players and trail it when they do (docs/BACKTEST.md); stash picks beat the starter they would replace in the 2025 backtest, but that result is weak evidence because the backtest's future-week projections contained later injury news (docs/BACKTEST.md); the stash rule is chosen mainly because it avoids stashing players the team cannot start; drop suggestions are rare and outscored the dropped player, but roughly tie the best free agent at the position; ir_stash adds points in an otherwise empty IR slot. The backtest's injury status was a snap-based stand-in, so injury-driven advice is the least tested.`;
