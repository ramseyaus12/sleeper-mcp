...
- How far to trust waiver buckets (2025 backtest, docs/BACKTEST.md): start_now pickups beat the starter they replace (+10 to +15 pts over 3 weeks, depending on whether other managers claim players) but only tie the top-projected free agent at the position; stash picks beat the starter they would replace over 3 weeks but trail the top-projected free agent at their own position; drop suggestions are rare and held up; ir_stash adds points in an otherwise empty IR slot. The backtest's injury status was a snap-based stand-in, so injury-driven advice is the least tested.`;
