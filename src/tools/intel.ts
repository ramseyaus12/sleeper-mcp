...
export function registerIntelTools(server: McpServer, ctx: ServerContext): void {
  ...
  server.registerTool(
    "get_waiver_targets",
    {
      ...
      description:
        "Waiver-wire recommendations for one team in a league: start_now (free agents projected to beat one of your starters in the target week), stash (free agents with the largest 3-week projected edge over the starter each would replace; never K or DEF), ir_stash (injured players worth an open IR slot), drop_candidates (bench players to drop, each with a better replacement, or IR moves) and bench_watch (highly ranked bench players who are slipping). Every pickup has a 3-week projection and a horizon for how long it should help. Uses league-scored projections, usage trends, merged injury status and Sleeper trending adds; every entry has plain-language reasons. No news is fetched.",
      inputSchema: {
