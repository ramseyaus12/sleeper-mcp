...
- `horizon` = how long the pickup should help, with a plain-language `horizon_reason`:
  ...
  - `rest_of_season`: the player's own usage label is `rising` or `breakout` over at least `THRESHOLDS.restOfSeasonMinWeeks` (3) played weeks, with no injury opportunity behind it; `played_weeks` shows the sample size. With fewer played weeks a stash falls back to `short_term` and a start_now to `this_week`.
...
- **`start_now`**: `start_gain` of at least `THRESHOLDS.minStartGain` (1.0 point, so fractional projection edges do not count) and designation not in `OUT_DESIGNATIONS` (Out, IR, PUP, Doubtful, Sus, NA). Uses the target week's projection only. Sorted by `start_gain`, with at most one K and one DEF (`THRESHOLDS.startNowKDefLimit`).
- **`stash`**: any other pool player (not in `start_now`) whose higher of the target week's and next week's projection is above 0 and at least 40% (`THRESHOLDS.stashProjShare`) of your weakest starter's projection at that slot, so a player on bye can still qualify (reason: "No game in week N; projects X pts in week N+1", or "Listed <designation> in week N; ..." when he has a designation). No injury opportunity or `rising` / `breakout` label is needed; usage and opportunity still appear in the reasons. K and DEF never go to stash: they are streamed through `start_now` (at most one of each), and the 2025 backtest did not cover them; the live run on 2026-09-24 returned three kickers among five stash picks before this rule. Sorted by `proj_next3` minus the 3-week projection (target week and the next two) of the starter the player would replace (the weakest eligible starter in your optimal lineup, as for `start_gain`), at most `THRESHOLDS.stashLimit` (5). This rule is variant V7 of the 2025 backtest (`docs/BACKTEST.md`). The earlier default (V6, sorted by raw `proj_next3`) returned five backup QBs for a 1-QB team in the live run, and 61% of its backtest picks were QBs. V7 beat it against the replaced starter in 12 of 12 combinations over 3 weeks (+11.5 vs -0.1 points), but trails it against the highest-projected free agent at the same position (-1.8 vs +2.4). V8 (V7 with at most one stash per position) was not chosen: its 3-week gain over the replaced starter (+4.5) was more than 2 points below V7's.
- **`ir_stash`**: unrostered players whose Sleeper designation is IR or PUP, when your team has an IR slot left open after its own IR moves (below) and their Sleeper `search_rank` is within `THRESHOLDS.irStashRank` (150). Only IR and PUP count as IR-eligible, because the league settings carry no `reserve_allow_*` flags. Each entry includes the merged status (its note carries return timelines) and `search_rank`. Sorted by `search_rank`, at most `THRESHOLDS.irStashLimit` (3).
- **`drop_candidates`** (from your bench), with safeguards so a good player is never dropped for a worse one:
  - Bench players in the optimal lineup for the target week are never drop candidates.
  - Sleeper designation IR or PUP: suggest an IR slot instead of a drop when one is open (`league.settings.reserve_slots`, else the IR entries in `roster_positions`, minus the players in `roster.reserve`); otherwise a drop candidate.
