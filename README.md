...
| `get_waiver_targets` | `start_now` (free agents projected to beat one of your starters this week), `stash` (best 3-week projections), `ir_stash` (injured players for an open IR slot), `drop_candidates` (each with a better replacement, or an IR move) and `bench_watch`. Every pickup has a 3-week projection, a `horizon` for how long it should help, and plain-language `reasons`. |
...
How far to trust `get_waiver_targets`: a replay of the 2025 season ([docs/BACKTEST.md](docs/BACKTEST.md)) found that `start_now` pickups beat the starter they replace but only tie the top-projected free agent at the position, `stash` roughly ties or slightly beats that free agent, drop suggestions are rare and held up, and `ir_stash` adds points in an otherwise empty IR slot. Injury status in the replay was a snap-based stand-in, so injury-driven advice is the least tested.
